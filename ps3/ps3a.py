...
def get_word_score(word, n):
    ...
    score = 0
    for a in word:
        score += SCRABBLE_LETTER_VALUES.get(a,0) * length        
    if n == length:
        score += 50
    return score    
#
...
def update_hand(hand, word):
    ...
    hand_update = {}
    for letter in hand.keys():
        hand_update[letter] = hand.get(letter,0) - word_dict.get(letter,0)
    return hand_update
...
def play_game(word_list):
    ...
    * If the user inputs anything else, ask them again.
    ...
    hand = {}
    while True:
        order = raw_input("'n', play a new (random) hand. 'r', play the last hand again. 'e', exit the game.")
        if order == 'n' or hand == {}:
